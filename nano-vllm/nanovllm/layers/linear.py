...
class LinearBase(nn.Module):
    def __init__(
        ...
        self.tp_dim = tp_dim
        self.tp_rank = dist.get_rank() if tp_dim is not None else None
        self.tp_size = dist.get_world_size() if tp_dim is not None else None
    ...
    def weight_loader(self, param: nn.Parameter, loaded_weight: torch.Tensor):
        raise NotImplementedError("Subclasses should implement this method to load weights.")
    
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        raise NotImplementedError("Subclasses should implement this method to define the forward pass.")
...
class ReplicatedLinear(LinearBase):
    ...
    def weight_loader(self, param: nn.Parameter, loaded_weights: torch.Tensor):
        # For replicated linear, we just copy the full weight
...
class ColumnParallelLinear(LinearBase):
    def __init__(
        ...
        assert output_size % tp_size == 0, "Output size must be divisible by the tensor parallel size."
