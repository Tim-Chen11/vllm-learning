...
if __name__ == "__main__":
    # Example usage
    x = torch.randn(8, 4000, 8000).cuda()  # Example input tensor with shape (batch_size, sequence_length, feature_dim)
    gamma = torch.full((8000,), 0.5, device="cuda", dtype=x.dtype)  # Example gamma tensor with shape (feature_dim,)
    layer = LayerNorm(gamma=gamma).cuda()
    residual = torch.full_like(x, fill_value=1.0)  # Example residual tensor with the same shape as x
